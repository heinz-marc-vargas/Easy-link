...
private
  ...
  def data
    products.map do |product|
      [
        link_to(product.name, edit_product_path(product, :from => "supplier"), :remote => true, :title => "Edit"),
  ...
  def actions(product)
    ...
    if can? :update, product
      actions =  link_to(raw("<i class='splashy-document_letter_edit'></i>"), edit_product_path(product, :from => "supplier"), :remote => true, :title => "Edit")
    end
  end
  
  def fetch_products
    products = Product.where("uid=0").order("#{sort_column} #{sort_direction}")
